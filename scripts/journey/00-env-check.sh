#!/usr/bin/env bash
# ── 第 1 个历程 · 环境体检 ────────────────────────────────────────────
# 本历程比较特殊:验证脚本本身就是教具——教程教你读懂它输出的体检报告。
# 必需工具(git/gcc/make)缺失 → 红;建议工具缺失只提示,不算失败。
# tier: ci-matrix
#
# 本脚本的运行方式:./scripts/journey/00-env-check.sh
#
# 正文示例命令(逐字收录,供 lint 对账;脚本自己不会去装):
...
#   交叉工具链与模拟器(第 6/7 个历程 才需要,现在可以先不装):
...
banner "建议工具(现在缺不要紧,后面的历程会用到再装)"
for tool in gdb cmake arm-none-eabi-gcc qemu-system-arm; do
  if command -v "$tool" >/dev/null 2>&1; then
    printf '  [ok] %s\n' "$tool"
  else
    printf '  [未装] %s —— 后面的历程会用到,到时候再装也来得及\n' "$tool"
  fi
done
